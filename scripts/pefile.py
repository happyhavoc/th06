...
Forked and backported to python 3.4

Carrera Ventura, E. (2024). pefile (Version 2024.8.26) [Computer software]. https://github.com/erocarrera/pefile
...
#
def parse_strings(data, counter, resources_strings):
    i = 0
    error_count = 0
    while i < len(data):
        ...
        if len_ != 0 and 0 <= len_ * 2 <= len(data):
            try:
                resources_strings[counter] = data[i : i + len_ * 2].decode("utf-16le")
...
@lru_cache(maxsize=2048)
def is_valid_function_name(s, relax_allowed_characters: bool = False) -> bool:
    allowed_extra = b"._?@$()<>"
...
class PE:
                            ...
                            for st_entry in entry.StringTable:
                                for key, entry in list(st_entry.entries.items()):
                                    ...
                                    if len(entry) > lengths[1]:
                                        resources_strings = entry.decode(
                                            "utf-8"
                                        ).encode("utf-16le")
                                        file_data[
                                            offsets[1] : offsets[1] + lengths[1] * 2
                                        ] = resources_strings[: lengths[1] * 2]
                                    else:
    ...
    def parse_sections(self, offset):
        ...
        for i in range(self.FILE_HEADER.NumberOfSections):
            ...
            if count_zeroes(section_data) == section.sizeof():
                self.__warnings.append(
                    "Invalid section {}. Contents are null-bytes.".format(i)
                )
                break
            if not section_data:
                self.__warnings.append(
                    "Invalid section {}. No data in the file (is this corkami's ".format(
                        i
                    )
                    + "virtsectblXP?)."
                )
            ...
            if section.SizeOfRawData + section.PointerToRawData > len(self.__data__):
                ...
                self.__warnings.append(
                    "Error parsing section {}. SizeOfRawData is larger than file.".format(
                        i
                    )
                )
            ...
            if self.adjust_PointerToRawData(section.PointerToRawData) > len(
                ...
                simultaneous_errors += 1
                self.__warnings.append(
                    "Error parsing section {}. PointerToRawData points beyond ".format(
                        i
                    )
                    + "the end of the file."
                )
            ...
            if section.Misc_VirtualSize > 0x10000000:
                simultaneous_errors += 1
                self.__warnings.append(
                    "Suspicious value found parsing section {}. VirtualSize is ".format(
                        i
                    )
            ...
            if (
                ...
                simultaneous_errors += 1
                self.__warnings.append(
                    "Suspicious value found parsing section {}. VirtualAddress is ".format(
                        i
                    )
                    + "beyond 0x10000000."
                )
            ...
            if (
                ...
                self.__warnings.append(
                    (
                        "Error parsing section {}. ".format(i)
            ...
            if section.__dict__.get(
                ...
                if section.Name.rstrip(b"\x00") == b"PAGE" and self.is_driver():
                    ...
                    self.__warnings.append(
                        "Suspicious flags set for section {}. ".format(i)
                ...
                if dir_entry.VirtualAddress:
                    if (
                        ...
                        try:
                            ...
                            self.__warnings.append(
                                'Failed to process directory "{}": {}'.format(
                                    entry[0], excp
                                )
                            )
                    if value:
    ...
    def parse_exceptions_directory(self, rva, size):
        ...
        for rf in rt_funcs:
            ...
            if rf.unwindinfo.FunctionEntry not in rva2rt:
                self.__warnings.append(
                    "FunctionEntry of UNWIND_INFO at {}".format(
                        rf.struct.get_file_offset()
                    )
            ...
            try:
                ...
                self.__warnings.append(
                    ...
                    + "{}: {}".format(rf.struct.get_file_offset(), excp)
                )
                continue
    ...
    def parse_relocations(self, data_rva, rva, size):
        ...
        try:
            ...
            self.__warnings.append("Bad RVA in relocation data: 0x{}".format(data_rva))
            return []
            ...
            if dbg.Type == 1:
                ...
                if dbg_type_data[:4] == b"RSDS":
                    ...
                    if dbg_type is not None:
                        ...
                        ).replace("-", "").upper() + str(dbg_type.Age)
                            ...
                            for resource_lang in resource_id.directory.entries:
                                ...
                                try:
                                    ...
                                    self.__warnings.append(
                                        ...
                                        + "RVA 0x{} with ".format(string_entry_rva)
                                        + "size {}".format(string_entry_size)
                                    )
                                    continue
                        ...
                        while (
                            ...
                            try:
                                ...
                                self.__warnings.append(
                                    ...
                                    + "unicode string at offset 0x{}".format(
                                        ustr_offset
                                    )
                                )
                                break
    ...
    def parse_export_directory(self, rva, size, forwarded_only=False):
        ...
        for i in range(min(export_dir.NumberOfNames, int(safety_boundary / 4))):
            ...
            if symbol_counts[(symbol_name, symbol_address)] > 10:
                self.__warnings.append(
                    ...
                    + "({}, {}). Assuming corrupt.".format(symbol_name, symbol_address)
                )
                break
        ...
        if not export_parsing_loop_completed_normally:
            self.__warnings.append(
                ...
                + "address: {}".format(export_dir.AddressOfNames)
            )
            ...
            if idx + export_dir.Base not in ordinals:
                ...
                if symbol_counts[symbol_address] > self.max_repeated_symbol:
                    ...
                    self.__warnings.append(
                        ...
                        + "{} ordinal entries. Assuming corrupt.".format(
        ...
        if not export_parsing_loop_completed_normally:
            self.__warnings.append(
                ...
                + "address: {}".format(export_dir.AddressOfFunctions)
            )
            return
        ...
        for entry in self.DIRECTORY_ENTRY_IMPORT:
            ...
            for imp in entry.imports:
                ...
                if not imp.name:
                    if not funcname:
                        raise PEFormatError(
                            "Unable to look up ordinal {}:{}".format(
    ...
    def parse_import_directory(self, rva, size, dllnames_only=False):
        ...
        while True:
            try:
                ...
                self.__warnings.append(
                    "Error parsing the import directory at RVA: 0x{}".format(rva)
                )
                break
            ...
            if not dllnames_only:
                try:
                    ...
                    self.__warnings.append(
                        ...
                        + "Invalid Import data at RVA: 0x{} ({})".format(
                            hex(rva), e.value
                        )
                    )
                ...
                if error_count > 5:
                    self.__warnings.append(
                        ...
                        "Invalid import data at RVA: 0x{}".format(hex(rva))
                    )
                    break
    ...
    def parse_imports(
        ...
        if (not iat or len(iat) == 0) and (not ilt or len(ilt) == 0):
            self.__warnings.append(
                ...
                + "OriginalFirstThunk: 0x{} ".format(hex(original_first_thunk))
                + "FirstThunk: 0x{}".format(hex(first_thunk))
            )
            return []
    ...
    def get_offset_from_rva(self, rva):
        ...
        if not s:
            ...
            raise PEFormatError("data at RVA 0x{} can't be fetched".format(hex(rva)))
        ...
        if hasattr(self, "VS_VERSIONINFO"):
            for idx, vinfo_entry in enumerate(self.VS_VERSIONINFO):
                if len(self.VS_VERSIONINFO) > 1:
                    dump.add_header("Version Information {}".format(idx + 1))
                else:
        ...
        if hasattr(self, "DIRECTORY_ENTRY_RESOURCE"):
            ...
            for res_type in self.DIRECTORY_ENTRY_RESOURCE.entries:
                if res_type.name is not None:
                    ...
                    dump.add_line(
                        "Id: [0x{}] ({})".format(hex(res_type.struct.Id), res_type_id),
                        2,
                    )
                ...
                if hasattr(res_type, "directory"):
                    ...
                    for resource_id in res_type.directory.entries:
                        if resource_id.name is not None:
                            ...
                            dump.add_line(
                                "Id: [0x{}]".format(hex(resource_id.struct.Id)), 6
                            )
    ...
    def adjust_SectionAlignment(self, val, section_alignment, file_alignment):
        ...
        if section_alignment < 0x1000:
            if (
                ...
                self.__warnings.append(
                    "If SectionAlignment(0x{}) < 0x1000 it should ".format(
                        hex(section_alignment)
                    )
                    + "equal FileAlignment(0x{})".format(hex(file_alignment))
                )
