...
DIFFABLE_STATIC_ARRAY(Enemy *, 11, g_Bosses);
DIFFABLE_STATIC_ARRAY(Enemy, 256, g_Enemies);
    ...
    YOLO:
        if ((ZunBool)(enemy->currentContext.time.current == instruction->time))
        {
            ...
            case ECL_OPCODE_BOSSSET:
                ...
                {
                    g_Bosses[instruction->args.setInt] = enemy;
                    g_Gui.bossPresent = 1;
            ...
            case ECL_OPCODE_SPELLCARDSTART:
                ...
                g_RunningSpellcardInfo.isCapturing = 1;
                g_RunningSpellcardInfo.isActive = 1;
                g_RunningSpellcardInfo.idx = instruction->args.spellcardStart.spellcardId;
                g_RunningSpellcardInfo.captureScore = g_SpellcardScore[g_RunningSpellcardInfo.idx];
                ...
                local_70 = &g_GameManager.catk[g_RunningSpellcardInfo.idx];
                csum = 0;
                if (!g_GameManager.isInReplay)
                {
                    ...
                    local_70->captureScore = g_RunningSpellcardInfo.captureScore;
            ...
            case ECL_OPCODE_SPELLCARDEND:
                if (g_RunningSpellcardInfo.isActive)
                {
                    ...
                    if (g_RunningSpellcardInfo.isActive == 1)
                    {
                        scoreIncrease = g_BulletManager.DespawnBullets(12800, 1);
                        if (g_RunningSpellcardInfo.isCapturing)
                        {
                            local_80 = &g_GameManager.catk[g_RunningSpellcardInfo.idx];
                            local_88 = g_RunningSpellcardInfo.captureScore >= 500000
                                           ? 500000 / 10
                                           : g_RunningSpellcardInfo.captureScore / 10;
                            scoreIncrease =
                                g_RunningSpellcardInfo.captureScore +
                                g_RunningSpellcardInfo.captureScore * g_Gui.SpellcardSecondsRemaining() / 10;
                    ...
                    g_RunningSpellcardInfo.isActive = 0;
                }
            ...
            case ECL_OPCODE_ENEMYKILLALL:
                for (local_b4 = &g_Enemies[0], local_b8 = 0; local_b8 < ARRAY_SIZE_SIGNED(g_Enemies);
                     local_b8++, local_b4++)
                {
